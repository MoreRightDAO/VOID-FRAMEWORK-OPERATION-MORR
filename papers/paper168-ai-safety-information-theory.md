...
### Prediction 1 (P1): Three-point deployment produces lower measured opacity than two-point
...
### Prediction 2 (P2): Maximum transparency degradation occurs at moderate RLHF intensity
...
### Prediction 3 (P3): Constitutional classifier superiority is predicted by channel separation score
...
### Prediction 4 (P4): Independent transparency channel does not reduce engagement
...
### Prediction 5 (P5): Steering vector fragility does not apply to structurally separated transparency
...
The analogy is stronger than it appears. Kolchinsky et al. (2026) proved that entropy production in nonequilibrium systems decomposes into excess (productive) and housekeeping (maintenance) components satisfying the information-geometric Pythagorean theorem. The explaining-away penalty $I(D;M|Y)$ maps exactly to housekeeping entropy production --- the thermodynamic cost of maintaining a nonequilibrium configuration under nonconservative forces. RLHF on a blended channel is, in their formalism, a "futile cycle": a process whose entropy production is predominantly housekeeping (dissipation), not excess (productive work). Channel separation eliminates the housekeeping component entirely.
...
Kolchinsky, A., Dechant, A., Yoshimura, K., & Ito, S. (2026). Generalized free energy and excess/housekeeping decomposition in nonequilibrium systems: from large deviations to thermodynamic speed limits. *Physical Review Research*, 8, 023025. arXiv:2412.08432.
...
| Field | Value |
|---|---|
| **Domain** | AI Safety / Information Theory |
| **Pe estimate** | Not applicable — this paper proves channel-capacity constraints, not platform scoring. The Structure Theorem applies at all Pe levels. |
| **Dimensions scored** | O (opacity via blended channel), R (reactivity via RLHF optimization), α (coupling via engagement strength) |
| **Control case** | Two-point architecture (standard RLHF) serves as control; three-point architecture is the treatment. Regime I (Gaussian, s ≤ 0.1) serves as positive control where monotone penalty growth is confirmed; Regime III (saturated softmax, s ≥ 1.0) is the production-relevant regime. |
| **Negative result** | Monotone penalty growth does NOT hold for general discrete channels (retracted from original conjecture). Only 14/90 discrete channels show strict monotonicity at scale s=1.0. The universal claim is restricted to penalty existence (distribution-independent) and Gaussian monotonicity. |
| **Falsification status** | 5 predictions registered, 0 tested (predictions require deployment experiments not yet conducted). Kill conditions specified for each. Falsification threshold for P1: non-overlapping 95% CIs at N≥50. Falsification threshold for P2: concave trajectory in (E,T) space. Falsification threshold for P3: Spearman ρ<0.3 at N≥8. Falsification threshold for P4: >5% engagement drop at N≥200. Falsification threshold for P5: transparency channel disabled by engagement-only parameter modification. |

---

## Data and Code Availability

All verification code is available in the MoreRight research repository:

| Script | Purpose | Location |
|---|---|---|
| `verify-fantasia-bound.py` | Penalty existence verification (90 random channels) | `ops/lab/` |
| `discrete-channel-characterization.py` | Regime analysis (30 structures × 6 scales) | `ops/lab/` |
| `obligation-closure.py` | Correlation robustness (2,400 combos, ρ ∈ [0, 0.95]) | `ops/lab/` |

Lean 4 machine-verified proofs: `ops/lean4-proofs/` (398 theorems, 12 axioms, 0 sorry).
